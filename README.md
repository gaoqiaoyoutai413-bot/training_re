...
就活などの外部公開用に、現在は `NEXT_PUBLIC_DEMO_MODE=true` を前提にしたデモ表示へ対応しています。

- Google ログインを使わず、そのまま画面を確認できます
- アプリ内で表示するユーザー名・メールアドレスはテストデータへ置き換えます
- Slack / Google Docs / Google Drive などの外部連携は停止します

通常運用へ戻す場合は `NEXT_PUBLIC_DEMO_MODE=false` を設定してください。
...
- [AIレビュー判定基準](docs/ai-review-criteria.md)
- [メンター向けレビュー運用マニュアル](docs/mentor-review-manual.md)
